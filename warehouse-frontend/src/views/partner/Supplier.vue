<template>
  <div class="page-shell">
    <div class="page-header">
      <div>
        <h2>供应商管理</h2>
        <p>维护供应商档案、联系方式和合作状态。</p>
      </div>
      <div class="soft-tag">合作伙伴</div>
    </div>

    <el-card class="data-card">
      <div class="panel-toolbar">
        <div class="left-tools">
          <el-input v-model="searchName" clearable placeholder="搜索供应商名称" style="width:220px;" @keyup.enter.native="handleSearch" />
    ...
    <el-dialog :title="dialogTitle" :visible.sync="dialogVisible" width="620px">
      <el-form ref="form" :model="form" :rules="rules" label-width="90px">
        <el-row :gutter="14">
          <el-col :span="12"><el-form-item label="编码" prop="supplierCode"><el-input v-model="form.supplierCode" /></el-form-item></el-col>
          <el-col :span="12"><el-form-item label="名称" prop="supplierName"><el-input v-model="form.supplierName" /></el-form-item></el-col>
        </el-row>
        <el-row :gutter="14">
...
export default {
  ...
  methods: {
    async loadData() {
      ...
      try {
        const res = await request.get('/supplier/page', {
          params: { current: this.current, size: this.size, supplierName: this.searchName }
        })
    ...
    handleDelete(id) {
      this.$confirm('确认删除该供应商吗？', '提示', { type: 'warning' }).then(async () => {
    ...
    handleSubmit() {
      this.$refs.form.validate(async valid => {
        ...
        if (this.form.id) {
          ...
          this.$message.success('供应商已更新')
        } else {
          await request.post('/supplier', this.form)
          this.$message.success('供应商已创建')
        }
