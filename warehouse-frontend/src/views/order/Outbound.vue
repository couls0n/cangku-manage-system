<template>
  <div class="page-shell">
    <div class="page-header">
      <div>
        <h2>出库单管理</h2>
        <p>登记出库订单、客户、仓库和发货状态。</p>
      </div>
      <div class="soft-tag">销售出库流程</div>
    </div>

    <el-card class="data-card">
      <div class="panel-toolbar">
        <div class="left-tools">
          <el-input v-model="searchOrderNo" clearable placeholder="搜索出库单号" style="width:220px;" @keyup.enter.native="handleSearch" />
      ...
      <el-table :data="tableData" stripe v-loading="loading">
        ...
        <el-table-column prop="orderTime" label="出库时间" width="170" />
    ...
    <el-dialog :title="dialogTitle" :visible.sync="dialogVisible" width="680px">
      <el-form ref="form" :model="form" :rules="rules" label-width="90px">
        <el-row :gutter="14">
          <el-col :span="12"><el-form-item label="出库单号" prop="orderNo"><el-input v-model="form.orderNo" /></el-form-item></el-col>
        ...
        <el-row :gutter="14">
          <el-col :span="12"><el-form-item label="客户" prop="customerId"><el-select v-model="form.customerId" style="width:100%;"><el-option v-for="item in customerList" :key="item.id" :label="item.customerName" :value="item.id" /></el-select></el-form-item></el-col>
        ...
        <el-row :gutter="14">
          <el-col :span="12"><el-form-item label="出库时间"><el-date-picker v-model="form.orderTime" type="datetime" style="width:100%;" value-format="yyyy-MM-dd HH:mm:ss" /></el-form-item></el-col>
...
export default {
  ...
  methods: {
    ...
    async bootstrap() {
      await Promise.all([this.loadWarehouse(), this.loadCustomer()])
      this.loadData()
    },
    async loadWarehouse() {
    ...
    async loadData() {
      ...
      try {
        ...
        this.tableData = res.data.records.map(item => ({
          ...
          customerName: (this.customerList.find(customer => customer.id === item.customerId) || {}).customerName || ''
        }))
    ...
    handleConfirm(row) {
      this.$confirm('确认将该出库单标记为已完成吗？', '提示', { type: 'warning' }).then(async () => {
        await request.put('/outbound', { ...row, status: 1 })
        this.$message.success('出库单已确认')
        this.loadData()
      }).catch(() => {})
    },
    handleDelete(id) {
      this.$confirm('确认删除该出库单吗？', '提示', { type: 'warning' }).then(async () => {
    ...
    handleSubmit() {
      this.$refs.form.validate(async valid => {
        ...
        if (this.form.id) {
          ...
          this.$message.success('出库单已更新')
        } else {
          await request.post('/outbound', this.form)
          this.$message.success('出库单已创建')
        }
